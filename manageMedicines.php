            ...
            <div class="container mt-4">
                <div class="row">
                    ...
                    <table class="table table-hover">
                                    ...
                                    <tbody id="myTable">
                                    <?php 
                                    $resultset = getMedList($con);
                                    $i = 1;
                                    while( $developer = mysqli_fetch_assoc($resultset) ) {
                                    ?>
                                        <tr>
                                            <td style="width: 10%"><?php echo date('Y-m-d', strtotime($developer ['date'])); ?></td>
                                            <td style="width: 65%" title="<?php echo $developer ['medicines']; ?>"><?php echo $developer ['medicines']; ?></td>
...
<?php
function getMedList($con){
    $resultset = null;
    if(isset($_GET['pID'])){
        $pID = $_GET['pID'];
        $sql_query = "SELECT m.id, m.date, m.medicines, u.name FROM medicine m INNER JOIN user u ON m.doctorID = u.id WHERE m.patientID = $pID ORDER BY m.date DESC;";
